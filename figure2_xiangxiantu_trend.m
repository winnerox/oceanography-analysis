...
% ========== 可自定义参数 ==========
% 离群值抽稀比例（可自由调节）
outlier_sampling_ratio = 0.05; % 30%的离群值显示，可根据需要调整
...
outlier_marker_size = 30;     % 离群值圆圈大小
...
% 字体设置
xlabel_fontsize = 14;         % 横轴字体大小
ylabel_fontsize = 14;         % 纵轴字体大小
title_fontsize = 14;          % 标题字体大小
tick_fontsize = 13;           % 刻度标签字体大小
% ================================

% 颜色设置
colors = [1 0 0; 0 0 1; 0 1 0]; % 红色、蓝色、绿色 - 用于离群值
box_color = [0.8 0.8 1]; % 箱体颜色
categories_3 = {'SSLA', 'TSLA', 'HSLA'};
categories_4 = {'TSLA(M1)', 'TSLA(M2)', 'HSLA(M1)', 'HSLA(M2)'};
...
for dataset_idx = 1:3
    ...
    if ~isempty(outliers)
        ...
        scatter(x_pos, outliers, outlier_marker_size, colors(dataset_idx, :), 'o', 'filled', 'MarkerEdgeColor', 'k', 'LineWidth', 1);
    end
end
...
for i = 1:3
    ...
    patch([x_left, x_right, x_right, x_left], [q1, q1, q3, q3], box_color, 'FaceAlpha', 0.7, 'EdgeColor', 'k');
    plot([x_left, x_right], [median_val, median_val], 'r-', 'LineWidth', median_line_width);
    ...
    plot([center, center], [q1, whisker_min], 'k-', 'LineWidth', whisker_line_width);
    plot([center, center], [q3, whisker_max], 'k-', 'LineWidth', whisker_line_width);
    scatter(center, mean_val, 100, 'k', 'filled', 'o', 'MarkerEdgeColor', 'k', 'LineWidth', 1);
end
...
for i = 1:3
    ...
    plot([center-0.1, center+0.1], [whisker_min, whisker_min], 'k-', 'LineWidth', percentile_line_width, 'Color', [0.3 0.3 0.3]);
    plot([center-0.1, center+0.1], [whisker_max, whisker_max], 'k-', 'LineWidth', percentile_line_width, 'Color', [0.3 0.3 0.3]);
end

ylabel('Trend difference (mm/yr)', 'FontSize', ylabel_fontsize);
set(gca, 'XTick', box_centers_3, 'XTickLabel', categories_3, 'FontSize', tick_fontsize);
grid on;

% 添加子图标签 a)
text(0.1, 0.98, '(a)', 'Units', 'normalized', 'FontSize', 18, 'FontWeight', 'bold', ...
...
steric_EN4_2 = [remove_extremes(clean_grids_EN4_c13(:, 4)); remove_extremes(clean_grids_EN4_c13(:, 5)); remove_extremes(clean_grids_EN4_c13(:, 6))];
thermo_EN4_2 = [remove_extremes(clean_grids_EN4_c13(:, 7)); remove_extremes(clean_grids_EN4_c13(:, 8)); remove_extremes(clean_grids_EN4_c13(:, 9))];
halo_EN4_2 = [remove_extremes(clean_grids_EN4_c13(:, 10)); remove_extremes(clean_grids_EN4_c13(:, 11)); remove_extremes(clean_grids_EN4_c13(:, 12))];

% 处理IAP数据 - 第4-12列
steric_IAP_2 = [remove_extremes(clean_grids_IAP(:, 4)); remove_extremes(clean_grids_IAP(:, 5)); remove_extremes(clean_grids_IAP(:, 6))];
thermo_IAP_2 = [remove_extremes(clean_grids_IAP(:, 7)); remove_extremes(clean_grids_IAP(:, 8)); remove_extremes(clean_grids_IAP(:, 9))];
halo_IAP_2 = [remove_extremes(clean_grids_IAP(:, 10)); remove_extremes(clean_grids_IAP(:, 11)); remove_extremes(clean_grids_IAP(:, 12))];

% 处理Ishii数据 - 第4-12列
steric_Ishii_2 = [remove_extremes(clean_grids_Ishii(:, 4)); remove_extremes(clean_grids_Ishii(:, 5)); remove_extremes(clean_grids_Ishii(:, 6))];
thermo_Ishii_2 = [remove_extremes(clean_grids_Ishii(:, 7)); remove_extremes(clean_grids_Ishii(:, 8)); remove_extremes(clean_grids_Ishii(:, 9))];
halo_Ishii_2 = [remove_extremes(clean_grids_Ishii(:, 10)); remove_extremes(clean_grids_Ishii(:, 11)); remove_extremes(clean_grids_Ishii(:, 12))];
...
% 添加子图标签 b)
text(0.1, 0.98, '(b)', 'Units', 'normalized', 'FontSize', 18, 'FontWeight', 'bold', ...
     'VerticalAlignment', 'top', 'HorizontalAlignment', 'left');
...
% 添加子图标签 c)
text(0.1, 0.98, '(c)', 'Units', 'normalized', 'FontSize', 18, 'FontWeight', 'bold', ...
     'VerticalAlignment', 'top', 'HorizontalAlignment', 'left');
...
% 添加子图标签 d)
text(0.1, 0.98, '(d)', 'Units', 'normalized', 'FontSize', 18, 'FontWeight', 'bold', ...
     'VerticalAlignment', 'top', 'HorizontalAlignment', 'left');
...
% 在主图上添加统一图例（放在顶部）
legend_elements = [
    plot(NaN, NaN, 'r-', 'LineWidth', 2, 'DisplayName', 'Median line'), ...
    plot(NaN, NaN, 'ko', 'MarkerFaceColor', 'k', 'MarkerSize', 8, 'DisplayName', 'Mean value'), ...
    plot(NaN, NaN, 'k-', 'LineWidth', 2, 'Color', [0.3 0.3 0.3], 'DisplayName', ' 2.5% ~ 97.5%'), ...
    plot(NaN, NaN, 'ro', 'MarkerFaceColor', 'r', 'MarkerSize', 8, 'DisplayName', 'Outliers-EN4'), ...
    plot(NaN, NaN, 'bo', 'MarkerFaceColor', 'b', 'MarkerSize', 8, 'DisplayName', 'Outliers-IAP'), ...
    plot(NaN, NaN, 'go', 'MarkerFaceColor', 'g', 'MarkerSize', 8, 'DisplayName', 'Outliers-Ishii')
];

% 在主图上添加图例
% 精细调整
hL = legend(legend_elements, 'Location', 'northoutside', 'Orientation', 'horizontal', ...
       'NumColumns', 6, 'FontSize', 14, 'Box', 'off');
