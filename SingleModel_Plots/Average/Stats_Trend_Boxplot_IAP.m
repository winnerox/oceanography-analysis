...
for n = 1:8
    ...
    for k = 1:num_valid
        ...
        vec_t(k) = p(1); % IAP Average 源数据通常只需要 p(1) (已经是 mm)
        % 如果源数据是 m，则需要 * 1000。IAP_TSLA_Average.m 输出的是 mm.
    end
    Data_Trend(:,n) = vec_t;
end
...
title(ax1, '\fontname{Microsoft YaHei}IAP (Average) 各阶趋势分布图 (0轴对齐版)', 'FontSize', 16, 'FontWeight', 'bold');
...
title('\fontname{Microsoft YaHei}IAP (Average) 各阶趋势分布图 (单轴)', 'FontSize', 16, 'FontWeight', 'bold');
