%% Step20_IAP_Map_Trend.m
% =========================================================================
% 功能: IAP 数据趋势空间分布图 (显著性打点 + 统一观感版)
% 修复: 
%   1. 移除多余的 *1000 单位转换 (数据本身已为 mm)
%   2. 增加 P 值计算与显著性打点 (p < 0.05)
%   3. 统一色棒与刻度显示
% =========================================================================
clear; clc; close all;
...
for n = 1:min(8, max_order)
    ...
    for i=1:nlon
        for j=1:nlat
            ...
            % 【修复】直接使用 p(1)，不再乘 1000 (因为 IAP 源数据已经是 mm)
            % 为了保险，增加智能检测：如果数值非常小 (<1e-3)，说明可能是 m，则乘 1000
...
sgtitle('\fontname{Microsoft YaHei}IAP 1-8 阶独立项趋势分布 (点区域 p < 0.05)', 'FontSize', 16, 'FontWeight', 'bold');
set(findall(gcf, '-property', 'FontName'), 'FontName', 'Times New Roman');

% 保存
OutputDir = 'D:\work\Figures';
OutFile = fullfile(OutputDir, 'IAP_Trend_Map_8Orders_Sig.jpg');
